...
```
.env.local.example        Copy to .env.local (gitignored) - needs ORS_API_KEY, DATABASE_URL
.github/
  workflows/
...
prisma/
  schema.prisma          Postgres schema: School, Route, RouteStep, Waypoint (see "Postgres" below)
  seed.ts                 `npx prisma db seed` - loads public/data/'s CSVs/JSON into Postgres
  migrations/              Generated by `prisma migrate dev` - one folder per migration
prisma7.config.ts          Prisma CLI config (connection string, seed command) - see its own
                              doc comment for why the connection string isn't in schema.prisma
src/
  app/
    ...
    api/route-master-list/route.ts Regenerates route-master-list.csv's schema from Postgres
    api/schools/route.ts   Regenerates schools.csv's schema from Postgres
    api/routes/[id]/steps/route.ts One route's steps sheet, from Postgres
    api/waypoints/route.ts  The shared geocode cache - GET reads it, POST persists a
                              freshly-resolved coordinate (see "Postgres" below)
  components/
  ...
  lib/
    ...
    db.ts                 Prisma client singleton (driver-adapter based - see "Postgres" below)
scripts/
...
Route/school/steps/waypoint-cache data now lives in Postgres (see
"Postgres: routes, schools, and the waypoint cache move off static
files" further below), not just `public/data/`'s CSVs/JSON directly -
copy `.env.local.example` to `.env.local`, fill in a real
`DATABASE_URL` (a local Postgres works fine for dev - e.g.
`postgresql://postgres:postgres@localhost:5432/shortstop` - so does a
free hosted one: Neon, Vercel Postgres, Supabase), then:
...
`migrate dev` creates the schema; `db seed` loads `public/data/`'s
CSVs/JSON into it (safe to re-run any time those files change - every
write is an upsert). Open http://localhost:3000. Works in any modern
browser; test the Bluetooth remote on an actual iPad/iPhone in Safari.
...
still refreshes itself automatically - a GitHub Actions workflow
...
so there's normally nothing to do by hand. Those JSON files stay the
source documents (`npm run geocode` doesn't talk to Postgres directly);
re-run `npx prisma db seed` after a fresh geocode run lands to pull the
new coordinates into the database the running app actually reads from.
To run the geocoder yourself anyway (a manual local check, or to retry
a failed lookup without touching a CSV): `npm run geocode`. Needs a
real `ORS_API_KEY` (addresses, and the one-time per-route anchor point
Overpass's intersection lookups use) and real outbound network access
to `api.openrouteservice.org` and `overpass-api.de` - not available in
every environment (this one included), so run it somewhere that has
both.
...
## Postgres: routes, schools, and the waypoint cache move off static files

  Route/school/steps data (`route-master-list.csv`, `schools.csv`,
  each route's own `{route}.csv`) and the geocode cache
  (`{route}-waypoints.json`) used to be exactly what the browser
  fetched at runtime - `page.tsx` pulled the CSVs directly from
  `public/data/` on load, `parseRouteCsv`/`parseRouteMasterList`/
  `parseSchoolsCsv` turned them into `Route`s client-side, and
  `RouteMap`/`StepScreen`/`EditRouteScreen` each fetched their own
  route's waypoint sidecar the same way. None of that parsing logic
  changed - what changed is where the *bytes* those parsers consume
  come from: a handful of new API routes
  (`src/app/api/route-master-list`, `/schools`, `/routes/[id]/steps`,
  `/waypoints`) now read from Postgres (`prisma/schema.prisma`:
  `School`, `Route`, `RouteStep`, `Waypoint`) and hand back text in the
  exact same shape those files always were, so every existing parser
  and every existing fetch call site just points at a new URL.

  The CSVs/JSON themselves aren't gone - they're still the district's
  own source documents, and now also this migration's seed input
  (`prisma/seed.ts`, `npx prisma db seed`): safe to re-run any time a
  source file changes, since every write is an upsert keyed on the
  same identity the file itself carries (route id, school name, a
  step's sequence within its route, a waypoint's own content-addressed
  cache key). `npm run geocode`'s GitHub Actions workflow still writes
  its results to the JSON sidecars exactly as before - re-seeding after
  a fresh geocode run is what actually gets new coordinates into the
  database the running app reads from.

  The waypoint cache stopped being split one file per route along the
  way - it's a single `Waypoint` table now, keyed by the same
  content-addressed `waypointCacheKey` it always used (an intersection
  or address resolves to the same key regardless of which route asked
  for it first), so a shared table is strictly more useful than the
  old per-route files with no change to how a lookup works. That also
  retired the one thing this migration was really unblocking:
  EditRouteScreen's "Fetch Location"/"Fetch All Locations" now POST
  every resolved coordinate straight to `/api/waypoints` the moment
  it's geocoded, so it's saved for every future load instead of only
  this session's - real persistence, not the "Download Coordinates"
  button that used to hand the admin a JSON file to drop into
  `public/data/` by hand (see "A stopgap: downloading the resolved
  coordinates" above). That button's gone now that what it was a
  stopgap *for* actually exists.

  Deliberately left alone: admin route edits themselves
  (`EditRouteScreen`'s Save) are still session-only, same known gap as
  before - this migration is about where the app's *read* data lives,
  not about adding a new write-back feature that isn't otherwise
  scoped. Postgres is a driver-adapter connection now (`src/lib/db.ts`,
