...
## Maps, part thirteen - Geoapify + MapLibre GL
  ...
  Merged into `development` as-is (still unproven against a real key/
  live network) rather than left drifting further on its own branch -
  explicitly *not* fast-forwarded to `main`, since it fully replaces
  CARTO/Leaflet and the live deploy has no `NEXT_PUBLIC_GEOAPIFY_API_KEY`
  configured yet; merging to `main` now would blank out the live map
  tiles rather than upgrade them. That's a deliberate later step once
  a real key exists in Vercel - tracked as a reminder, not forgotten.
