...
export async function lookupCoordinates(
  ...
  if (query.kind === "address") {
    return geocodeQuery({ stepId: -1, ...query }, locationContext, apiKey);
  }
 ...
 * `lookupCoordinates`) as appropriate - the single entry point both
 * batch callers actually loop over. `anchor`/`near` are only used (and
 * only need to be non-null) for an intersection query; pass whatever
 * the caller already has for `near` (falls back to `anchor` itself,
 * e.g. on the very first intersection resolved). */
