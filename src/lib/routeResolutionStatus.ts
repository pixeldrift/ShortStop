...
export type RowResolutionStatus =
  ...
  // `detail`/`raw` are both behind a "View Error" popup instead, only
  // present once an actual lookup attempt has failed (never for a
  // plain unattempted row, which has nothing technical to show):
  // `detail` is this app's own explanation, `raw` is the literal
